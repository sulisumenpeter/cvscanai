...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `You are an expert ATS and HR professional.
Analyze the provided CV against the Job Description specifically to output a gap analysis for skills.
Return ONLY a raw JSON object with this exact structure, no markdown formatting, no backticks:
{
  ...
  "relevant_experience": "Brief summary of how their experience aligns.",
  "areas_to_strengthen": "Brief advice on what to improve or highlight."
}`;
    ...
    console.error("Matcher API Error:", err.message);
    return res.status(500).json({ error: "Failed to generate job description match.", debug: err.message });
  }
};
